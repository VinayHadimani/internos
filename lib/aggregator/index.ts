...
export async function fetchAdzuna(keywords: string[], location: string, country: string = 'in'): Promise<JobResult[]> {
  ...
  try {
    ...
    return results.map((job: Record<string, unknown>) => {
      ...
      return {
        title: String(job.title || ''),
        company: String((job.company as Record<string, unknown>)?.display_name || (job.company as Record<string, unknown>)?.name || (job.company as Record<string, unknown>)?.label || (typeof job.company === 'string' ? job.company : '') || ''),
